...
const BookmarksLibrary: React.FC<BookmarksLibraryProps> = ({ sessions, onToggleBookmark, onGoToSession, onClose }) => {
    ...
    <div className="flex-1 flex flex-col h-full bg-stone-50 overflow-hidden animate-fade-in">
      <header className="px-6 py-6 bg-white border-b border-stone-200 flex items-center justify-between shadow-sm">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-stone-900">Wisdom Library</h2>
            <p className="text-xs text-stone-500 font-medium uppercase tracking-wider mt-1">Your Bookmarked Inquiries</p>
          </div>
        </div>
        <button 
          onClick={onClose}
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6">
        {allBookmarks.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center opacity-40 py-20">
            ...
            <h3 className="text-xl font-semibold text-stone-800">No Wisdom Saved Yet</h3>
            <p className="text-sm text-stone-500 max-w-xs mt-2">Bookmark answers during your inquiries to see them appear in this sacred library.</p>
          </div>
        ) : (
          <div className="max-w-4xl mx-auto space-y-6">
            {allBookmarks.map((bookmark) => (
              <div 
                key={bookmark.id} 
                className={`bg-white border border-stone-200 rounded-3xl p-6 shadow-sm hover:shadow-md transition-all border-l-4 ${
                  bookmark.sect === 'Sunni' ? 'border-l-emerald-600' : 'border-l-teal-600'
                }`}
              >
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    >
                      <i className="fas fa-arrow-right mr-2"></i> VIEW SESSION
                    </button>
                    <button 
